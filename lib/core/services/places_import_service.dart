...
class PlacesImportService {
  ...
  Future<void> _importPlacesForCity(String cityName, double latitude,
    ...
    try {
      ...
      final duration = endTime.difference(startTime);
      print(
          '  📊 Tìm thấy ${places.length} địa điểm trong ${duration.inMilliseconds}ms');

      // ✅ Lọc địa điểm du lịch chất lượng
      final filteredPlaces = _filterTouristPlaces(places);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'OK') {
          ...
          print('    ✅ Nearby Search thành công: ${results.length} kết quả');
          return results;
        } else {
          print(
